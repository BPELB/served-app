...
function SponsoredCard({ ad, onSelect, isDark, photos }) {
  ...
  const photoUrl = photoIdx < pool.length ? thumbUrl(pool[photoIdx]) : null;
  return (
    <div style={{background:OBG,border:"2px solid #16a34a" /* dark mode's own accent, reused as the lighter-green border tint in both themes since O===OBG in light mode */,borderRadius:18,marginBottom:10,overflow:"hidden",cursor:"pointer"}}
...
function ImageSlider({ seed, type, subtype }) {
  ...
  return (
    <div style={{position:"relative",marginBottom:16}}>
      ...
      <button onClick={()=>scroll(-1)} aria-label="Previous photo" style={{...arrowStyle,left:-16}}>
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"/></svg>
      </button>
      <button onClick={()=>scroll(1)} aria-label="Next photo" style={{...arrowStyle,right:-16}}>
...
function BusinessPage({ business, onBack, onRate }) {
  ...
  const handleShare = async () => {
    const shareData = { title: business.name, text: `Check out ${business.name} on GreenChek`, url: window.location.href };
    if (navigator.share) {
      ...
      <div style={{padding:"1rem"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          ...
          <div onClick={()=>onAdvertise()} style={{padding:"16px 18px",background:OBG,
            border:"2px solid #16a34a" /* dark mode's own accent, reused as the lighter-green border tint in both themes since O===OBG in light mode */,borderRadius:18,display:"flex",alignItems:"center",gap:14,cursor:"pointer"}}>
            <div style={{width:46,height:46,borderRadius:13,
              background:BG,
              display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
